  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-orbit-dark/70 backdrop-blur-3xl border border-white/10 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-14 shadow-2xl relative group overflow-hidden min-h-auto md:min-h-[500px] flex flex-col justify-center items-center"
      >
        ...
        {/* Visual Density Layer: Noise Texture */}
        ...
        {/* Visual Density Layer: Ambient Orbs */}
        <motion.div
          animate={{
            scale: [1, 1.2, 1],
            opacity: [0.3, 0.5, 0.3],
...
const StickySection = ({ feature, index }: { feature: any; index: number }) => {
  ...
  return (
    ...
    >
      ...
      {/* 3D Card Content */}
      <ThreeDCard className="w-full max-w-3xl mx-4 md:mx-auto" index={index}>
        <div className="text-center w-full">
          ...
          >
            <Sparkles
              className="absolute -top-4 -left-4 md:-top-5 md:-left-6 text-orbit-gold animate-pulse"
              size={16}
            />
            <span className="text-orbit-gold text-xs md:text-sm tracking-[0.2em] md:tracking-[0.3em] uppercase font-bold border-b border-orbit-gold pb-1 md:pb-2 drop-shadow-[0_0_10px_rgba(235,189,97,0.5)]">
              Chapter 0{index + 1}
            </span>
          </motion.div>
          ...
          <motion.div
            ...
            className="relative px-4 py-4 md:px-6 md:py-6 border-t border-b border-white/10 bg-white/5 backdrop-blur-sm rounded-xl mb-6 md:mb-8"
          >
            <p className="text-gray-100 text-base md:text-lg leading-relaxed font-light drop-shadow-lg">
              {feature.desc}
            </p>
          </motion.div>
          ...
          {index === 1 && (
            <motion.div
              ...
              className="grid grid-cols-2 gap-4 md:gap-6 mt-6 md:mt-8"
            >
              {[
                ...
                >
                  <div className="p-2 md:p-3 bg-white/5 rounded-xl md:rounded-2xl border border-white/20 group-hover/icon:bg-orbit-gold group-hover/icon:text-black group-hover/icon:scale-110 transition-all duration-300 shadow-lg shadow-white/5 backdrop-blur-sm">
                    <item.icon size={18} />
                  </div>
                  <span className="text-[10px] md:text-[10px] uppercase tracking-widest font-bold text-gray-400 group-hover/icon:text-white transition-colors">
                    {item.label}
                  </span>
                </div>
              ))}
